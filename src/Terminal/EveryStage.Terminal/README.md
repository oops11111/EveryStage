...
38. **`SettingsPanel`的"显示"标签页里，显示器列表只在面板构造时枚举一次**（`PopulateMonitorComboBox`
    在构造函数里调用，没有刷新按钮）——`MainWindow.ShowPanel`把每个面板实例都长期复用，不是每次
    导航都重新构造，所以"重新点一次设置导航"并不会重新枚举，实际需要重启整个终端机主界面才能看到
    刚插拔的显示器。写这份README时发现面板上原来的文字提示写的是"重新进入设置页面即可看到最新
    列表"，这个措辞是错的（会让人以为切换导航就够了）——已经在这一轮改成准确描述实际行为的文案；
    真正实现"重新进入面板时刷新列表"需要让`MainWindow`不再对所有面板都永久复用同一个实例，这个
