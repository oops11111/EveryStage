...
113. **【已实现一部分，原为已知缺口】"启动时没绑定扩展屏、运行中途插入新显示器"——原来完全
    ...
    如果启动时没有绑定扩展屏，这个事件从来不会被订阅，`HandleDisplaySettingsChanged`也就永远
    没有机会跑——现在改成无条件订阅（`OnExitRequested`里对应的取消订阅也同步改成无条件）。
    `HandleDisplaySettingsChanged`新增一个分支：`_overlay == null`时，如果
    `MonitorService.GetBoundExtendedDisplay`现在返回非空（说明启动之后有新显示器接入），
    通过新增的`TrayIconController.ShowNotification`（`NotifyIcon.ShowBalloonTip`的薄封装——
    这是一个长期稳定不变的WinForms API，风险等级跟这个仓库大量D3D11/Media Foundation互操作
    完全不是一个量级，不需要额外的"未验证"顾虑）弹一次气泡提示"检测到扩展屏...请重启终端机以
    启用扩展屏输出"。用`_notifiedDisplayAvailableAfterStartup`这个字段保证只提醒一次，不会
    在之后每次别的显示器分辨率变化时都重复弹。**顺手修正一处过时注释**：构造函数里原来声称
    "the tray tooltip below reflects it"（没有绑定扩展屏这件事，托盘tooltip会反映出来），
    但`TrayIconController.UpdateTooltip`从来只读`OutputStateMachine.State`/`CastSwitchOn`，
    根本不知道`_overlay`是否存在——这句话从写下来那一刻起就不是真的，这次顺手改正。
    **明确没有做的部分（跟第59条(a)的理由完全一样，这次没有重新评估、只是补上这条更早的提醒）**：
    仍然没有在运行时凭空搭建`_overlay`/`_videoSurface`/`_playback`/`_previewWindow`这一整套
    对象图——插上新显示器之后，操作者仍然需要手动重启终端机才能真正用上它，这条只是把"重启"
    这件事从"操作者要么自己发现要么永远不知道"变成"至少弹窗告诉你需要重启"。**没有做的部分**：
...
117. **【已实现，应用户要求继续做，原为已知缺口】PLANNING.md §11"批量选择"的最后一半——"统一
    ...
    **确认下来的设计**：每一行属性默认"不修改"，操作者必须主动勾选"修改此项"才会把这一项应用到
    ...
    主题下可读）完全没有验证过。
118. **【已实现，应用户要求继续做，原为产品决策空白】跨活动自动播放——一个活动顺序自动播完
    最后一个文件后，现在会自动接着播方案里的下一个活动**：`Activity.cs`自己的doc comment（见
    该文件、以及"尚未开始"曾经列出的那条）早就点名这是PLANNING.md没写清楚、需要产品决策才能
    动手的空白——这次直接用`AskUserQuestion`问了用户两个具体问题，再照确认下来的设计实现，
    没有自己猜。**确认下来的设计**：(1) 播完方案里最后一个活动之后，停在最后一项，不循环回
    方案第一个活动——跟`CompletionAction.Loop`只针对单个文件的循环语义分开，不会把整个方案
    变成一个大循环；(2) 悬浮预览窗"上一项/下一项"这两个手动按钮（`PlaybackTrigger.ManualSkip`）
    刻意不跨活动边界，只有真正播放完成触发的自动前进（`PlaybackTrigger.ActivityAuto`）才会跨
    ——这样手动操作的行为范围保持跟"到目前为止一直如此"一致，不会因为这次改动突然多出一种
    "点一下按钮就跳去另一个活动"的新行为。**实现方式**：新增`PlaybackEngine.TryAdvanceToNextActivity`
    ，只在`HandleCompletion`的`CompletionAction.NextItem`分支、`TryAdvance(1, ActivityAuto)`
    自己在当前活动内部找不到下一项时才被调用——按方案里的顺序继续往后找，跳过没有可播放内容的
    活动（空活动，或者活动里全是背景音频——这种活动会被这次改动透明跳过，就跟`TryAdvance`本来
    就已经透明跳过单个背景音频文件一样，途中遇到的背景音频仍然会被启动，只是不会真正"停留"在
    那个活动上），落到下一个真正可播放的活动的第一个文件上才真正切换`_currentActivity`/
    `_currentFileIndex`——如果一路找到方案末尾都没有，这两个字段完全不被改动，效果就是"停在
    原来那个活动的最后一项"。**新增的依赖**：`PlaybackEngine`构造函数新增`ScenarioStore`参数
    （`Program.cs`那行`new PlaybackEngine(...)`调用同步更新传入已有的`_store`）——这是这个类
    第一次需要知道`Scenario`这一层，之前它拿到的都只是某个`Activity`本身，靠一次引用相等的
    `_scenarioStore.Scenarios.FirstOrDefault(s => s.Activities.Contains(_currentActivity))`
    反查它属于哪个方案，跟`ActivitiesPanel.TryHighlightPlayingFile`用引用相等而不是按名字查找
    `MediaFile`是同一种思路。`Activity.cs`那句曾经"夸大"的doc comment（"顺序是权威播放顺序"）
    这次真正改成准确描述现状，而不是像上次那样只是把夸大的说法改成"目前不是"。**没有做的部分**：
...
- ~~`Activity`类自己的doc comment曾经声称"`Scenario.Activities`里的顺序是`PlayMode.SequentialAuto`
  的权威播放顺序"，这句话夸大了实际行为~~ **【产品决策空白已确认、已实现，见第118条】**：跨活动
  自动播放当时判断"到底该不该做、要做成什么交互，仍然是产品决策空白"，这次直接问了用户并确认
  了具体设计（只对自动播完触发，不对手动上一项/下一项生效；播完方案最后一个活动后停住、不循环）
  再动手实现。
