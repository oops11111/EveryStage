...
41. **离线条目没有"移除配对"入口**：跟Terminal那边`DevicesPanel`有专门的"移除配对"按钮不同，
    Caster这个待机面板本身很紧凑（PLANNING.md §12"单一任务导向，不做复杂功能堆叠"），这一轮没有
    为此新增UI——一个不会再出现的旧终端机会永久留在离线列表里，唯一的清除办法是直接删除
    `caster-paired-terminals.json`（默认路径见`PairedTerminalStore`构造函数）。这是有意收窄的
    范围，不是遗漏，但如果这份列表在真实使用中積累了很多"再也不会上线"的僵尸条目，产品上大概率
    还是需要补一个移除入口。
