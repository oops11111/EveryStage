# EveryStage.Caster — 投屏机（阶段2一部分：发现/配对 + 屏幕捕获）

对应 `docs/PLANNING.md` §12 的UI描述、§7 的设备发现/配对流程的**投屏机一侧**，以及阶段2"屏幕捕获：
Desktop Duplication API (DDA)"这一项。**不包含**H.264编码、RTP推流、音频采集——PLANNING.md §15
把整个捕获/编码/传输称为"第二大技术风险区"，这里只做完了其中风险相对独立、可以单独验证的第一块
（捕获），编码和传输还是空白。
...
这些都是围绕一个真实存在的视频流设计的UI。在编码/传输管线存在之前，搭建这些UI元素只是在为不存在
的功能画界面，所以这里的"投屏中"面板只做了协议握手结果的诚实反馈 + 一个独立的捕获自检工具。
...
- H.264 硬件编码（Media Foundation 编码器会话）
- RTP 传输（丢包恢复NACK/FEC、拥塞控制）——真正把捕获到的画面发送给已配对的终端机
- 音频采集与同步
- 已配对设备的持久化列表
