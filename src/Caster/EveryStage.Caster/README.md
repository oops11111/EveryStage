...
84. **【新发现的真实bug，已修复】`LiveCastSession.RunLoop`的`finally`块调用`_cts?.Cancel()`时，
    ...
    **检查过的相邻类**：`H264HardwareEncoder`/`CaptureSelfTestRunner`/`EncodeSelfTestRunner`/
    ...
    构造出一次真正触发它的复现。**【第85条后续更新，重要修正】**上面这条"不是反复出现的
    通用模式"的结论只对"循环自己的`finally`直接触碰`_cts`字段"这一种具体机制成立——同一次
    审计后来扩大范围之后，发现`TerminalDiscoveryClient`（这里刚刚点名"确认过没问题"的那个
    类）其实中了另一种机制完全不同、但同样源于"`Dispose()`里`Wait`超时不检查就照样往下
    执行"的竞争条件，见第85条。这条更正本身就是"自己检查过的结论也可能不完整"的一个具体
    例子，不是要否定上面的分析——两种竞争条件确实是不同的代码路径，只是恰好都源于同一类
    "清理代码没检查`Wait`是否真的等到"的疏忽。

85. **【新发现的真实bug，已修复，跟`EveryStage.Transport`/`EveryStage.Terminal`同一次审计一起
    找到】`TerminalDiscoveryClient.ReceiveLoopAsync`没有捕获`ObjectDisposedException`，理论上
    能让接收循环变成没人观察的未处理Task异常**：这个类自己的`Dispose()`是`_cts.Cancel()`之后
    `_receiveLoop?.Wait(TimeSpan.FromSeconds(2))`等接收循环退出，同样没有检查这次等待到底
    成功还是超时，超时了也照样执行`_socket.Dispose()`——跟`EveryStage.Transport`的
    `RtpReceiver`/`RawRtpReceiver`（见该项目README对应条目的完整理由）以及
    `EveryStage.Terminal`的`DiscoveryService`是完全同一种结构、同一种竞争条件，只是这次审计
    范围扩大到了这个仓库全部四个"取消令牌+后台接收循环"类之后才发现这个类也中招——见第84条
    自己的更正说明，那条当时专门检查过这个类，但检查的是另一种（没有中招的）竞争机制，
    这次发现的是第三种、单独的一个。**修复方式**：给`ReceiveLoopAsync`补上跟另外三个类
    完全一样的`catch (ObjectDisposedException) { return; }`。**没有做的部分**：这次改动
